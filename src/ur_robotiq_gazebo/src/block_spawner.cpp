...
void srv_connection_check()
{
    ...
    while (!service_ready) 
    {
        ...
        ROS_INFO("waiting for delete_model service");
        ros::Duration(0.5).sleep();
    }
    ROS_INFO("delete_model service is ready");
...
spawn_model_req.initial_pose.position.x = 1.2;  // block start position
...
while (ros::ok())
{
    ...
    spawn_model_req.initial_pose.position.y = -0.8;
    ...
    bool del_service = deleteClient.call(delete_model_req, delete_model_resp);
    if (del_service)
    {
        if (delete_model_resp.success) {
            ROS_INFO_STREAM(model_name << " has been deleted");
        }
        else {
            ROS_INFO_STREAM(model_name << " delete failed");
        }
    }
    else 
    {
