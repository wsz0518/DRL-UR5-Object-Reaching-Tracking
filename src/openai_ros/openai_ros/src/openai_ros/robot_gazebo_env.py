...
class RobotGazeboEnv(gym.Env):
    ...
    def __init__(self, robot_name_space, controllers_list, reset_controls, start_init_physics_parameters=True, reset_world_or_sim="SIMULATION"):
        ...
        # Lock for Fine positioning
        self.info_lock = False
    ...
    def step(self, action):
        ...
        self.gazebo.unpauseSim()
        moved = self._set_action(action)
        self.gazebo.pauseSim()
        obs = self._get_obs()
        done = self._is_done(obs, moved)
        if self.info_lock is False:
            self.info_lock = self._get_info() # {}
        reward = self._compute_reward(obs, done, self.info_lock, moved)
        self.cumulated_episode_reward += reward

        rospy.logdebug("END STEP OpenAIROS")

        return obs, reward, done, self.info_lock
    ...
    def testGrasp(self):
        self.gazebo.unpauseSim()
        rospy.sleep(1)
        s1 = self._set_gripper_status(0)
        s2 = self._set_gripper_status(1)
        s3 = self._set_gripper_status(0)
        return s1, s2, s3
