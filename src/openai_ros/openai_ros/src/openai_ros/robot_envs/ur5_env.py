...
class UR5Env(robot_gazebo_env.RobotGazeboEnv):
    ...
    def __init__(self, ros_ws_abspath):
        ...
        self._shoulder_pan = rospy.Publisher(
            '/ur5/shoulder_pan_joint_position_controller/command',
            Float64, queue_size=1)
        self._shoulder_lift = rospy.Publisher(
            '/ur5/shoulder_lift_joint_position_controller/command',
            Float64, queue_size=1)
        self._elbow = rospy.Publisher(
            '/ur5/elbow_joint_position_controller/command',
            Float64, queue_size=1)
        self._wrist_1 = rospy.Publisher(
            '/ur5/wrist_1_joint_position_controller/command',
            Float64, queue_size=1)
        self._wrist_2 = rospy.Publisher(
            '/ur5/wrist_2_joint_position_controller/command',
            Float64, queue_size=1)
        self._wrist_3 = rospy.Publisher(
            '/ur5/wrist_3_joint_position_controller/command',
            Float64, queue_size=1)
        # self._gripper = rospy.Publisher(
        ...
        rospy.Subscriber("/ur5/joint_states",
        ...
        while self.base_position is None and not rospy.is_shutdown():
            try:
                self.base_position = rospy.wait_for_message(
                    "/ur5/joint_states", JointState, timeout=1.0)
                rospy.logdebug(
                    "Current ur5/joint_states READY=>" \
                ...
                rospy.logwarn("Current ur5/joint_states not ready yet, \
                               retrying for getting joint_states")

        rospy.logdebug("Connection to ur5/joint_states established")
    ...
    def wait_for_completion(self, joint, target, timeout):
        ...
        if elapsed > timeout:
            rospy.logerr(joint + ' could not reach desired position.')
            return False
        return True
    ...
    def move_shoulder_pan_joint(self, joints_dict):
        ...
        return self.wait_for_completion('shoulder_pan_joint', \
    ...
    def move_shoulder_lift_joint(self, joints_dict):
        ...
        # Stop further execution until it reaches the pose
        return self.wait_for_completion('shoulder_lift_joint', \
    ...
    def move_elbow_joint(self, joints_dict):
        ...
        # Stop further execution until it reaches the pose
        return self.wait_for_completion('elbow_joint', \
    ...
    def move_wrist_1_joint(self, joints_dict):
        ...
        # Stop further execution until it reaches the pose
        return self.wait_for_completion('wrist_1_joint', \
    ...
    def move_wrist_2_joint(self, joints_dict):
        ...
        # Stop further execution until it reaches the pose
        return self.wait_for_completion('wrist_2_joint', \
    ...
    def move_wrist_3_joint(self, joints_dict):
        ...
        # Stop further execution until it reaches the pose
        return self.wait_for_completion('wrist_3_joint', \
                    joints_dict['wrist_3_joint'], 3)
    
    def move_finger_joint(self, joints_dict):

        # joint_value = Float64()
        # joint_value.data = joints_dict['finger_joint']
        # # Publish desired pose
        # self._gripper.publish(joint_value)

        traj = JointTrajectory()
        traj.joint_names = ['finger_joint']
        point = JointTrajectoryPoint()
        point.positions = [joints_dict['finger_joint']]  # 设置目标位置
        point.time_from_start = rospy.Duration(1)  # 设置运动时间
        traj.points.append(point)
        self._gripper.publish(traj)

        # Stop further execution until it reaches the pose
        return self.wait_for_completion('finger_joint', \
                    joints_dict['finger_joint'], 5)
